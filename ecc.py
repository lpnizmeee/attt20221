...
def encrypt_ECC(msg, pubKey):
    ...
    return (ciphertext, nonce, authTag, ciphertextPubKey, ciphertextPrivKey)

def decrypt_ECC_admin(encryptedMsg, privKey):
    (ciphertext, nonce, authTag, ciphertextPubKey, ciphertextPrivKey) = encryptedMsg
    sharedECCKey = privKey * ciphertextPubKey
    secretKey = ecc_point_to_256_bit_key(sharedECCKey)
    plaintext = decrypt_AES_GCM(ciphertext, nonce, authTag, secretKey)
    return plaintext

def decrypt__ECC_user(encryptedMsg, pubKey):
    (ciphertext, nonce, authTag, ciphertextPubKey, ciphertextPrivKey) = encryptedMsg
...
encryptedMsgObj = {
    ...
    'ciphertextPrivKey': hex(encryptedMsg[4])
}
print("encrypted msg:")
